...
public class IpGeneratorUtils {
    ...
    public static String getNextIp(String ipAddress) {
        String[] attr = ipAddress.split("\\.");
        int incrBy = 1;
        for (int i = 3; i >= 0; i--) {
            int netCount = Integer.parseInt(attr[i]) + incrBy;
            incrBy = 0;
            if (netCount > 255) {
                incrBy = 1;
                netCount = netCount - 256;
            }
            attr[i] = String.valueOf(netCount);
        }
        return Joiner.on(".").join(attr);
    }
    ...
    public static boolean isInRange(String ip, String startIp, String endIp) {
        long ip2Long = ipToLongValue(ip);
        long startIp2Long = ipToLongValue(startIp);
        long endIp2Long = ipToLongValue(endIp);
        return ip2Long >= startIp2Long && ip2Long <= endIp2Long;
    }

    public static long ipToLongValue(String ip) {
        String[] attr = ip.split("\\.");
        return Long.parseLong(attr[0]) * 1000000000L + Long.parseLong(attr[1]) * 1000000L + Long.parseLong(attr[2]) * 1000 + Long.parseLong(attr[3]);

    }


    public static void main(String[] args) {
        System.out.println(getNextIp("192.168.0.255"));
        isInRange("192.168.8.3", "192.168.9.10/22");
        long ii = 192L << 24;
        System.out.println(ii);
        System.out.println(isInRange("192.168.0.2", "192.168.0.1", "192.168.0.250"));
        System.out.println(Long.MAX_VALUE);
        long aa = 192 * 1000000000L + 168 * 1000000L + 0 * 1000 + 250;
